...
async def show_keymap_obj_info(keymap: xkb.Keymap):
    ...
    for keycode in keycodes:
        ...
        for shift_level in range(keymap.num_levels_for_key(keycode, 0)):
            try:
                ...
                # Retrieve list of modifier indices for the current level
                mod_indices = keymap.key_get_mods_for_level(keycode, 0, shift_level)
                debug(f'{mod_indices = }')
                mod_names = [keymap.mod_get_name(idx) for idx  in mod_indices if idx < keymap.num_mods()]
                debug(f'{mod_names = }')

                # # Debug output to check modifiers at each level
                # print(f"Keycode {keycode}, Level {shift_level}: Modifiers - {mod_names}")

                # Don't show anything if there is no symbol for the key on this level in the layout
                if symbol_names:
                    symbol_data.append({
                        "level": shift_level + 1,       # Level indexing starts at 1 in descriptions
    ...
    for keycode, info in keycode_info.items():
        ...
        for level_info in info["levels"]:
            print(f"   Lvl{level_info['level']} S: {level_info['symbols']} M: {level_info['modifiers']}", end='   ')
