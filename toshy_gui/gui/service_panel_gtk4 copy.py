...
try:
    from toshy_gui.gui.help_dialog_gtk4 import HelpButton, HELP_CONTENT
except ImportError:
...
class ServicePanel(Gtk.Box):
    ...
    def create_status_section(self):
        ...
        if self.parent_window and HelpButton:
            help_content = HELP_CONTENT.get('service_status', {})
            help_button = HelpButton(
                self.parent_window,
                help_content.get('title', 'Service Status Help'),
                help_content.get('text', 'Information about Toshy service status.')
            )
    ...
    def create_controls_section(self):
        ...
        # Add header with help for service controls
        if self.parent_window and HelpButton:
            service_header_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
            service_header_box.set_halign(Gtk.Align.CENTER)
            service_header_box.set_margin_bottom(8)
            
            service_header = Gtk.Label(label="All Services")
            service_header.add_css_class("control-group-header")
            service_header_box.append(service_header)
            
            restart_help = HelpButton(
                self.parent_window,
                HELP_CONTENT.get('restart_services', {}).get('title', 'Restart Services'),
                HELP_CONTENT.get('restart_services', {}).get('text', 'Restart all Toshy services.')
            )
            service_header_box.append(restart_help)
            
            service_frame.append(service_header_box)
        ...
        # Add header with help for config-only controls
        if self.parent_window and HelpButton:
            config_header_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
            config_header_box.set_halign(Gtk.Align.CENTER)
            config_header_box.set_margin_bottom(8)
            
            config_header = Gtk.Label(label="Config Only")
            config_header.add_css_class("control-group-header")
            config_header_box.append(config_header)
            
            config_help = HelpButton(
                self.parent_window,
                HELP_CONTENT.get('config_only', {}).get('title', 'Config-Only Mode'),
                HELP_CONTENT.get('config_only', {}).get('text', 'Run only the config service.')
            )
            config_header_box.append(config_help)
            
            config_frame.append(config_header_box)
